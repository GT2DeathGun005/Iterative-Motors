...
- **Salvataggio anti-interruzione**: il checkpoint principale, la policy corrente, i buffer `.npz` e i sidecar `.txt` vengono scritti prima su file temporaneo, sincronizzati su disco e poi sostituiti con rename atomico. Prima di ogni sostituzione vengono mantenute due copie recenti (`.bak` e `.prev`) nella cartella dedicata `train_set/checkpoints/backups/`, preservando la stessa struttura relativa dei file originali. `Ctrl+C`/`SIGTERM` sono gestiti come richiesta di stop pulito: l'episodio viene chiuso al primo step utile e viene salvato un checkpoint completo. Se TORCS/Xvfb smette di rispondere via UDP, il client chiude il socket dopo timeout ripetuti invece di bloccare il processo all'infinito. Al resume il loader prova `main → .bak → .prev` e usa il primo checkpoint completo valido; i buffer più nuovi del `.pth` caricato vengono ignorati in favore del backup allineato, così un'interruzione tra salvataggio buffer e commit checkpoint non crea disallineamento.
...
- **Uscita di consolidamento**: se il breakout è vicino al miglior deterministico assoluto già preservato (`td3_det_best_dist`, margine 5m), la refinement termina subito anche se non supera il record di oltre la soglia anti-jitter. In alternativa, **3 eval buone consecutive** sopra la soglia di breakout (`eval > plateau_ref * 1.10`) bastano per consolidare: il peso Behavioral Cloning torna a `1.0`, il Critic viene riattivato e l'Actor può essere congelato temporaneamente per riallineare la Value Function. Questo evita di lasciare il Critic spento dopo aver già ottenuto un miglioramento stabile ma non ancora vicino al record assoluto.
...
- **Anti-loop per plateau**: massimo **3 tentativi** di refinement sullo stesso plateau; oltre, il training prosegue normale con Critic e Actor attivi. Se più avanti la mediana del plateau sale di almeno il 10%, il contatore viene azzerato perché è considerato un nuovo regime, non lo stesso muro. La logica della macchina a stati è validata offline (plateau→attiva, collasso→rollback, ancora-in-salita→non attiva, bimodale→nessun rollback spurio).
