...
- **Aggiornamento del Critic / aggiornamento dell'Actor**: ogni episodio logga esplicitamente se i rispettivi update sono attivi. `Aggiornamento Critic: disattivato (refinement: loss solo diagnostica)` significa che la loss del Critic è calcolata solo per monitoraggio: non viene eseguito né `backward()` né `optimizer.step()` sul Critic. `Aggiornamento Actor: disattivato (riscaldamento iniziale del Critic)` o `disattivato (Actor congelato)` spiega perché la loss dell'Actor può restare a `0.000`.
...
- **Actor Freezing Temporaneo**: L'Actor viene congelato (`agent.actor_frozen = True`) per un periodo stabilito di **30 episodi** dal momento del ripristino.
...
- **Scongelamento Sicuro**: Trascorsi i 30 episodi di stabilizzazione, l'Actor viene sbloccato (`agent.actor_frozen = False`), riprendendo l'addestramento TD3+BC con gradienti stabili e costruttivi.
