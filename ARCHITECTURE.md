...
- **Mutual Exclusion Fisica Unificata**: Per allineare il training online e il testing deterministico, la regola di esclusione reciproca rigida (acceleratore forzato a `0.0` se il freno supera `0.05`) è integrata direttamente nel ciclo di interazione con l'ambiente in `td3_bc.py` oltre a essere presente in `test_agent.py`.
