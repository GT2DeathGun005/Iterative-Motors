...
- **Entropia Costante (Fixed Alpha)**: Contrariamente allo standard SAC robotico, l'Auto-Tuning dell'Entropia (Alpha) è stato **disabilitato**. Nelle simulazioni di guida ad alta velocità, la policy ottima richiede azioni "bang-bang" (tutto gas, tutto freno), ovvero valori ai bordi del dominio `[-1, 1]`. A causa dello Jacobiano della funzione `tanh` usata da SAC, queste azioni generano penalità numeriche infinite (`log_pi` tendente a `+inf`). L'Auto-Tuning reagiva gonfiando Alpha a dismisura, punendo l'agente per aver accelerato e costringendolo allo stallo (azione zero). Alpha è ora **fissato a `0.02`**, garantendo un'esplorazione stabile senza esplosioni di gradiente.
- **Ambiente Esplorativo (Anti-Stall Relaxed)**: Per proteggere l'esplorazione SAC nei primissimi secondi di un episodio, il motore fisico di `TorcsEnv` è stato allentato. L'antistallo originario (che uccideva l'episodio se l'auto non superava i 20 km/h in 3 secondi) è stato portato a **10 secondi e 5 km/h**. Questo permette alla rete, inizialmente incerta, di "scoprire" delicatamente i pedali senza subire terminazioni punitive e falsi positivi.
