...
> **Mappatura dai vecchi nomi** (in caso si incontrino nei log storici): `td3_best_eval_laptime`→`td3_det_best_lap` · `td3_best_ever`→`td3_det_best_dist` · `td3_best_eval`→`td3_det_best_dist_run` · `td3_best_lap`→`td3_expl_best_lap` · `td3_best_dist`→`td3_expl_best_dist`.
...
- **🛡️ Rete di sicurezza (auto-rollback)**: se durante la refinement l'eval crolla sotto il **60% del riferimento di plateau** (la **MEDIANA** della finestra recente — più robusta del max al singolo picco stocastico) per **3 valutazioni consecutive**, l'Actor viene **ripristinato da `td3_det_best_dist.pth`**, il `bc_weight` torna a `1.0` e il Critic si scongela. La policy migliore non si perde MAI (è sempre su disco).
