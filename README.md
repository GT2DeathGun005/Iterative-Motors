...
#### Riproducibilità, Rumore ed Esplorazione (`--sigma` e `--seed`)

Nel Reinforcement Learning tradizionale (SAC), l'esplorazione è gestita intrinsecamente dal parametro `log_std` appreso dalla rete tramite massimizzazione dell'entropia. Tuttavia, a causa del **Warm Start (Behavioral Cloning)**, permettere alla SAC di gestire l'entropia causerebbe un'immediata esplosione del `log_std`, distruggendo i pesi del pilota umano (*catastrophic forgetting*). 

Per proteggere l'apprendimento base, l'Actor SAC è mantenuto in modalità quasi-deterministica (con `log_std` bloccato a -5.0). L'esplorazione necessaria al RL per scoprire traiettorie migliori viene invece aggiunta tramite un **rumore gaussiano esterno (`sigma`)** strettamente controllato e decrescente. 

Poiché i giri migliori spesso avvengono grazie a questa esplorazione *prima* che la rete abbia completamente internalizzato la traiettoria nella sua media deterministica, il `test_agent.py` supporta la replica esatta del rumore:

1. **Sigma automatico**: caricando un checkpoint completo (`sac_checkpoint_best.pth`), il test recupera in automatico il `sigma` esatto usato durante quell'episodio di addestramento.
2. **Seed search**: il test usa seed casuali per esplorare il rumore e stampa il seed di ogni tentativo (es. `Seed: 12345`).
3. **Replica deterministica**: quando l'agente completa un ottimo giro in test grazie a un seed fortunato, puoi riprodurlo al millimetro quante volte vuoi fissandolo:

```bash
# Replica perfetta e deterministica di un giro rumoroso di successo
...
| 18 | **L'Actor non impara a evitare gli ostacoli (Actor Blindness)** | L'aggiornamento del RL ("Advantage-Weighted CPI") usava una "maschera binaria" (`adv_mask`) per aggiornare la policy *solo* se l'azione dell'Actor era *migliore* di quella del buffer (`Q_actor > Q_buffer`). Questo azzerava i gradienti negativi (`adv_mask=0`), impedendo all'Actor di "imparare dagli errori" (non veniva punito per azioni disastrose come sbattere al muro). Quando `bc_lambda` è sceso sotto 0.5, l'Actor era libero di vagare ma "cieco" alle conseguenze, causando schianti sistematici. | **Rimozione Maschera Binaria**: Rimosso l'uso di `adv_mask` nel calcolo della loss di Q-improvement in `sac_rl.py`. Ora l'Actor riceve gradienti negativi che lo "spingono via" in modo naturale dalle traiettorie con Q-value pessimo, tornando a imparare la dinamica di collisione e a schivare i muri attivamente. |
